...
.defines(function() {
	EntitySoundBarButton = ig.Entity.extend({
		...
		pausePage:null,
		settingPage:null,
		barImage:new ig.Image('media/graphics/button/scroll-bar.png'),
		backImage:new ig.Image('media/graphics/button/back-bar.png'),
		buttonImage:new ig.Image('media/graphics/button/scroll-button.png'),
		testSound:"testSound",
		...
		draw:function(){
			...
			if(this.enable===true){
				...
				this.backImage.draw(this.pos.x,this.pos.y + 9.5*2,[0],[0],[329*2],[31*2]);
				
				if  (this.levle<1){
					this.barImage.draw(this.pos.x,this.pos.y + 9.5*2 ,[0],[0],2,[31*2]);
				}else if (this.level>329){
					this.barImage.draw(this.pos.x,this.pos.y + 9.5*2,[0],[0],329*2,[31*2]);
				}else{
					this.barImage.draw(this.pos.x,this.pos.y + 9.5*2,[0],[0],[this.level],[31*2]);
				}
				this.buttonImage.draw(this.pos.x + this.level - 25*2,this.pos.y - 12.5*2 + 9.5*2);
		...
		update:function(){
			...
			if (this.enable===true){
				...
				if (this.pausePage.enable===true){
					...
					if (ig.ua.mobile){
						this.pos.x = this.pausePage.pos.x + (1080 - 898)/2 + 86.5*2;
						this.pos.y = this.pausePage.pos.y + (1920 - 348)/2  - 60*2 + 29*2;
					}else{
						this.pos.x = this.pausePage.pos.x + (1920 - 898)/2 + 86.5*2;
						this.pos.y = this.pausePage.pos.y + (1080 - 348)/2  - 40*2 + 29*2;
					}
					
				}else if (this.settingPage.enable===true){

					if (ig.ua.mobile){
						this.pos.x = this.settingPage.pos.x + (1080 - 898)/2 + 86.5*2;
						this.pos.y = this.settingPage.pos.y + (1920 - 348)/2  - 60*2 + 29*2;
					}else{
						this.pos.x = this.settingPage.pos.x + (1920 - 898)/2 + 86.5*2;
						this.pos.y = this.settingPage.pos.y + (1080 - 348)/2  - 40*2 + 29*2;
					}
