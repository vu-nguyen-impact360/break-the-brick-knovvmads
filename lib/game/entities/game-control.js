...
.requires(
	...
	'game.entities.buttons.music-toggle',
	'game.entities.buttons.sound-toggle'
)
.defines(function() {
    EntityGameControl = ig.Entity.extend({
        ...
        init:function(x,y,settings){
			...
			this.pauseButton = ig.game.spawnEntity(EntityPauseButton, 106, 28, {gameControl:this})
			...
			this.ballNumber = 0;
			this.player = ig.game.spawnEntity(EntityPlayer,ig.system.width/2 - 117 , 1000,{gameControl:this, pointer: this.pointer });
			...
			*/
			...
			// wall
			if (ig.ua.mobile){
				this.rightWall = ig.game.spawnEntity(EntityWallSide, 1000 + 80, 0,{gameControl:this,rightSide:true});
				
			}else{
				this.rightWall = ig.game.spawnEntity(EntityWallSide, 1840 + 80, 0,{gameControl:this,rightSide:true});
			}
			this.leftWall = ig.game.spawnEntity(EntityWallSide, -80, 0,{gameControl:this,rightSide:false});
			this.upWall = ig.game.spawnEntity(EntityWallUp,0,-80,{gameControl:this});
			
			//page
			this.resultPage = ig.game.spawnEntity(EntityResultPage,0,-1080,{gameControl:this});
			if (ig.ua.mobile){
				this.pausePage = ig.game.spawnEntity(EntityPausePage,0,1920,{gameControl:this});
			}else{
				this.pausePage = ig.game.spawnEntity(EntityPausePage,0,1080,{gameControl:this});
			}
			...
			if (ig.ua.mobile){
				this.settingPage = ig.game.spawnEntity(EntitySettingPage,0,1920,{gameControl:this});
			}else{
				this.settingPage = ig.game.spawnEntity(EntitySettingPage,0,1080,{gameControl:this});
			}
			this.tutorialPage =  ig.game.spawnEntity(EntityTutorialPage,0,0,{gameControl:this});
			
			// sound  ,  sfx   BAR
			// this.soundBar = ig.game.spawnEntity(EntitySoundBarButton,0,0,{gameControl:this, pointer: this.pointer });
			// this.musicBar = ig.game.spawnEntity(EntityMusicBarButton,0,0,{gameControl:this, pointer: this.pointer });
			this.soundBtn = ig.game.spawnEntity(EntityMusicToggle, 0, -500, {gameControl: this});
			this.musicBtn = ig.game.spawnEntity(EntitySoundToggle, 0, -500, {gameControl: this});
        ...
        draw:function(){
			...
			ig.system.context.font = "20px Oswald";   // set font
