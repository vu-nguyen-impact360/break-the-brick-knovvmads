ig.module('game.entities.opening')
.requires(
    'impact.entity'
)
.defines(function () {
    EntityOpening = ig.Entity.extend({
		...
		kittyOpening: false,	// to control using kitty opening or not , default is false
		...
		ready: function(){
			...
			try{
				if(this.kittyOpening){
					createjs.Sound.play('kittyopening');	
				}else{
		...
		drawKittyOpening: function(){
			...
			this.kittyImage.drawTile(ig.system.width/2 - this.kittyImage.width/8,ig.system.height/2 - this.kittyImage.height/4,this.kittyAnim,217,349);
			ig.system.context.globalAlpha = 1 ;	
		},
