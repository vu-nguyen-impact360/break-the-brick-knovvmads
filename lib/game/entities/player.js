...
.defines(function() {
    EntityPlayer = ig.Entity.extend({
        zIndex:230,
        pos:new Vector2(0,0),
        size:new Vector2(234,46),
        color:new ColorRGB(125,255,125,1),
		animSheet: new ig.AnimationSheet('media/graphics/game/player.png',234,46),
        ...
        draw:function(){
			...
			if (this.fire===true){
				...
				this.gunImage.draw(this.pos.x + this.size.x/4 - 18,this.pos.y - this.gunDeltaPosY);
				this.gunImage.draw(this.pos.x + this.size.x/4 + this.size.x/4 + this.size.x/4 - 18,this.pos.y - this.gunDeltaPosY);
				...
				if (this.gameControl.checkButton.pressed===true){
						...
						if (this.pos.x < this.pointer.pos.x - this.size.x/2){
							...
							if (this.pos.x > this.pointer.pos.x -this.size.x/2){
								...
								if (this.pos.x > ig.system.width - 80 - this.size.x){
									this.pos.x = ig.system.width - 80 - this.size.x;
								}
							}
		...
		scaling:function(){
			...
			if (this.expandTimeCount>700){ //300 - 20 = 280
				...
				// fix bug scaling out 
				if (ig.ua.mobile){
					if (this.pos.x + this.size.x > (1080 - 80)){
						this.pos.x = (1080-80) - this.size.x;
					}
				}else{
					if (this.pos.x + this.size.x > (1920 - 80)){
						this.pos.x = (1920-80) - this.size.x;
					}
				}
		...
		firing:function(){
			...
			if (this.fireTime>=this.fireRate){
				...
				ig.game.spawnEntity(EntityBullet,this.pos.x + this.size.x/4 - 17,this.pos.y - 34,{gameControl:this.gameControl})
				ig.game.spawnEntity(EntityBullet,this.pos.x + this.size.x/4 + this.size.x/4 + this.size.x/4 - 17,this.pos.y - 34,{gameControl:this.gameControl})
		...
		reStart:function(){
			...
			this.pos.x = ig.system.width/2 - 117;
			this.pos.y = 1000;
