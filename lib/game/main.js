...
.defines(function(){	
	...
	MyGame = ig.Game.extend({	
		...
		initSfx: function(){
			//play sfx example   ---    ig.game.staticSound.play(); OLD Version
			//Go to impact/sound-handler.js to adjust
		...
		/* Please head to impact/sound-handler.js
		...
		/* no longer used since ig.soundHandler handles all sounds and music
		...
		*/			
		...
		/* use ig.soundHandler for your muting needs, any modifications can be done there
		pressMute:function(){
			if(ig.Sound.enabled){
		...
		*/
		...
		update:function(){
	        ...
	        // only update some of the entities when paused:
				...
				if(ig.soundHandler) // A win phone fix by yew meng added into ig.soundHandler
				{
					ig.soundHandler.forceLoopBGM();
					/* To be added into soundHandler if this works rather then Yew Meng's new fix
					if(ig.ua.mobile) {
						if(ig.game.jukebox) {
							if(ig.game.jukebox.player) {
								for (var s in ig.game.jukebox.player.settings.spritemap) {
									if( ig.game.jukebox.player.getCurrentTime() <=
										(
											ig.game.jukebox.player.settings.spritemap[s].end 
											//- ig.game.jukebox.player.settings.timeout/1000
										 ) ){
											//console.log(ig.game.jukebox.player.getCurrentTime());
											//continue;
										}
									else {
										ig.game.jukebox.player.pause();
