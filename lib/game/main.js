...
.defines(function(){	
	...
	MyGame = ig.Game.extend({	
		...
		draw: function(){
			this.parent();
			//Optional - to use , debug console , e.g : debugCL("debug something");
			//this.drawDebug();
		},
			...
			if(!ig.global.wm){
				...
				if(this.debug && this.debug.length > 0){
					if(!this.debugSetup){
						...
						this.debugW = 24 ;
						this.debugH = 24 ;
						this.debugX = ig.system.width/2 - this.debugW/2 ;
						this.debugY = 0
					}
					//check if mouse overlaping debug
					this.debugMP();
					if(this.viewDebug){
						//draw debug bg
						...
						ig.system.context.fillRect(this.debugX,this.debugY,this.debugW,this.debugH);
						...
						//debug minized , draw small button at top left
						ig.system.context.globalAlpha = 0.35 ;
						//debug background color				
						ig.system.context.fillStyle = 'white';
						ig.system.context.fillRect(this.debugX,this.debugY,this.debugW,this.debugH);
					...
					// delete console log 1 by 1 per second 
					if(!this.debugTimer){
						this.debugTimer = new ig.Timer(1);
		...
		debugCL: function(consoleLog){ //-----debug console log-----
		...
		debugMP: function(){	//-----debug mouse position-----
			//set debug button
			//get mouse position
			if(ig.ua.mobile){
				var currentMousePosX = ig.input.mouse.x / widthRatio ;
				var currentMousePosY = ig.input.mouse.y / heightRatio ;
			}else{
				var currentMousePosX = ig.input.mouse.x / multiplier ;
				var currentMousePosY = ig.input.mouse.y / multiplier ;
			}
			if(currentMousePosX < this.debugX)return ;
			if(currentMousePosX > this.debugX + this.debugW)return ;
			if(currentMousePosY < this.debugY)return ;
			if(currentMousePosY > this.debugY + this.debugH)return ;
