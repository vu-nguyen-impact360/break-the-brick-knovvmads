...
ig.Entity = ig.Class.extend({
	...
	//Hardcoded mobile positions for reposition upon device orientation
	posMP: {x:0, y:0},
	posML: {x:0, y:0},
	...
	//This function is called within repos.js
	//To enable this function, set this.enableReposition to true
	reposition:function(){
		//override this function if another reposititon logic is needed.
		if(ig.ua.mobile){
			if(this.enableReposition){
				if(portraitMode){
					this.pos.x = this.posMP.x;
					this.pos.y = this.posMP.y;
				}else{
					this.pos.x = this.posML.x;
