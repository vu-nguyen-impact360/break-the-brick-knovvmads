...
.defines(function () {
    ...
    ig.SoundHandler = ig.Class.extend({
		...
		*/		
		...
		{
			if(this.bgmStarted && this.bgmPlaying && !this.muted && !this.forceMuted)
			{
				if(this.jukebox) 
				{
					if(this.jukebox.player) 
					{
						if(this.jukebox.player.getCurrentTime() >= 0) {
							// all
							...
							// reset
							if(this.pausePosition) this.jukebox.player.setCurrentTime(this.pausePosition);
							else this.jukebox.player.setCurrentTime(this.jukebox.player.settings.spritemap.music.start);
						}
					}
				}
			}
